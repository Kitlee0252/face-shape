...
**Goal:** Add 10 SEO-targeted blog posts to faceshapeai.org using a single data-driven template — new posts require only a data entry, no code changes.

**Architecture:** One dynamic route `app/blog/[slug]/page.tsx` renders all posts from `lib/data/blogPosts.ts`. The template has conditional sections (shapeBreakdown, comparison, steps) that render only when data is present. A listing page at `app/blog/page.tsx` shows all posts in a card grid.

**Tech Stack:** Next.js 16 (App Router, SSG), TypeScript, TailwindCSS 4, pnpm

**Spec:** `docs/superpowers/specs/2026-04-08-blog-system-design.md`
...
|--------|------|---------------|
| Create | `lib/data/blogPosts.ts` | Blog post types + all 10 post data entries |
| Create | `app/blog/[slug]/page.tsx` | Blog post template (SSG, conditional sections, SEO) |
| Create | `app/blog/page.tsx` | Blog listing page (card grid, SSG) |
| Modify | `components/shared/Header.tsx:6-9` | Add "Blog" nav link |
| Modify | `app/sitemap.ts` | Add blog URLs to sitemap |

---

## Task 1: Blog Data Types + First Post

**Files:**
- Create: `lib/data/blogPosts.ts`

- [ ] **Step 1: Create blog data file with types and first post**
...
Run: `cd /home/projects/face-shape && pnpm exec tsc --noEmit --pretty 2>&1 | head -20`

Expected: No errors related to `blogPosts.ts`.

- [ ] **Step 3: Commit**

```bash
git add lib/data/blogPosts.ts
git commit -m "feat(blog): add blog data types and first post (face-shape-and-haircut)"
```

---

## Task 2: Blog Post Page Template

**Files:**
- Create: `app/blog/[slug]/page.tsx`

- [ ] **Step 1: Create blog post page template**
...
git commit -m "feat(blog): add blog post page template with conditional sections"
```
...
git commit -m "feat(blog): add blog listing page with card grid"
```

---

## Task 4: Header Navigation + Sitemap

**Files:**
- Modify: `components/shared/Header.tsx:6-9`
- Modify: `app/sitemap.ts`

- [ ] **Step 1: Add Blog link to Header navigation**

In `components/shared/Header.tsx`, change the `navLinks` array from:
...
- [ ] **Step 2: Update sitemap to include blog posts**

Replace `app/sitemap.ts` contents with:
...
```bash
git add components/shared/Header.tsx app/sitemap.ts
git commit -m "feat(blog): add Blog nav link and blog URLs to sitemap"
```
...
Run: `cd /home/projects/face-shape && pnpm run build 2>&1 | tail -30`

Expected: Build succeeds. Output should show blog routes generated:
- `/blog` (static)
- `/blog/face-shape-and-haircut` (static)

- [ ] **Step 2: Dev server spot check**

Run: `cd /home/projects/face-shape && pnpm run dev &` then check:
- `http://localhost:3000/blog` — listing page renders with 1 card
- `http://localhost:3000/blog/face-shape-and-haircut` — post renders with all sections
...
## Task 6: Add Blog Posts 2-4 (celebrity, glasses, sunglasses)

**Files:**
- Modify: `lib/data/blogPosts.ts`

- [ ] **Step 1: Add celebrity-face-shapes post**

Add to the `BLOG_POSTS` array in `lib/data/blogPosts.ts`:
...
- [ ] **Step 2: Add glasses-for-face-shape post**

Add to the `BLOG_POSTS` array:
...
- [ ] **Step 3: Add sunglasses-for-face-shape post**
...
Run: `cd /home/projects/face-shape && pnpm run build 2>&1 | tail -20`

Expected: Build succeeds with 4 blog routes.

- [ ] **Step 5: Commit**

```bash
git add lib/data/blogPosts.ts
git commit -m "feat(blog): add celebrity, glasses, and sunglasses posts"
```

---

## Task 7: Add Blog Posts 5-7 (personality, makeup, beard)

**Files:**
- Modify: `lib/data/blogPosts.ts`

- [ ] **Step 1: Add face-shape-and-personality post**
...
- [ ] **Step 2: Add face-shape-makeup-contouring post**
...
- [ ] **Step 3: Add beard-for-face-shape post**
...
Expected: Build succeeds with 7 blog routes.

- [ ] **Step 5: Commit**

```bash
git add lib/data/blogPosts.ts
git commit -m "feat(blog): add personality, makeup, and beard posts"
```

---

## Task 8: Add Blog Posts 8-10 (hat, oval-vs-round, how-to-determine)

**Files:**
- Modify: `lib/data/blogPosts.ts`

- [ ] **Step 1: Add hat-for-face-shape post**
...
- [ ] **Step 2: Add oval-vs-round-face post (comparison type)**
...
- [ ] **Step 3: Add how-to-determine-face-shape post (steps type)**
...
Expected: Build succeeds with all 10 blog routes plus listing page.

- [ ] **Step 5: Commit**

```bash
git add lib/data/blogPosts.ts
git commit -m "feat(blog): add hat, oval-vs-round, and how-to-determine posts"
```

---

## Task 9: Final Build + Dev Server Verification

- [ ] **Step 1: Full build**

Run: `cd /home/projects/face-shape && pnpm run build 2>&1 | tail -40`

Expected: Build succeeds. Blog routes should show:
```
├ /blog                              (static)
├ /blog/beard-for-face-shape         (static)
├ /blog/celebrity-face-shapes        (static)
├ /blog/face-shape-and-haircut       (static)
├ /blog/face-shape-and-personality   (static)
├ /blog/face-shape-makeup-contouring (static)
├ /blog/glasses-for-face-shape       (static)
├ /blog/hat-for-face-shape           (static)
├ /blog/how-to-determine-face-shape  (static)
├ /blog/oval-vs-round-face           (static)
├ /blog/sunglasses-for-face-shape    (static)
```

- [ ] **Step 2: Dev server spot checks**

Start dev server and verify:
1. `/blog` — 10 post cards rendered, sorted by date
2. `/blog/face-shape-and-haircut` — shapeBreakdown renders 7 shape cards
3. `/blog/oval-vs-round-face` — comparison table renders correctly
4. `/blog/how-to-determine-face-shape` — numbered steps render
5. Header nav shows "Blog" link
6. View page source: JSON-LD `BlogPosting`, `FAQPage`, `BreadcrumbList` present
7. Breadcrumbs: Home / Blog / {title} links work
